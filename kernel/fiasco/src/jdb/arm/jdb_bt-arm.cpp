...
/* jdb_bt-ia32-ux.cpp adapted for ARM */
...
Jdb_bt::cmds() const
{
  static Cmd cs[] =
	...
	{ 0, "bt", "backtrace", " [a]ddr/[t]hread",
	  ...
	  &dummy },
	/*
	 * The "bt" command only works with the help of the "b" command,
	 * which is currently not registered by another module on ARM
	 */
	{ 1, "b", "b_dummy", "%c",
	  "b\tdummy command to make the 'bt' command work in the\n"
	  "\tabsence of a real 'b' command",
	  &dummy },
    };
  return cs;
}
